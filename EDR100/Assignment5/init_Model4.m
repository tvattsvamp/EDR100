...
%  v = speed in m/s
%  V = speed in km/h
load('ACC_cycle2');   % Load mat-file called "WLTC_cycle", contining, t,v,V
...
EM_scaling = 1.8;        % Scale EM up or down. I.e. 2 makes the torque capability twice as big
...
PEC_scaling = 2.0;     % Scale of PEC power. 1.0 corresponds to ca 150 kW PMSM or IPM traction machine
...
ESS_cells_parallel = 58;    % How many cells in parallel?
ESS_cell_Cvalue = 2.5;      % Max relative peak current (C-rate). 1-3 C for EV packs, 5-10 C for PHEV packs.
...
m_v = 1832;                % (kg) Vehicle total mass
rotating_mass = 0;         % (%) Equivalent inertia (wheels, shafts) as percentage of total vehicle mass
d_wheel = 2.101/pi*0.97;   % (m) Tire unloaded circumference to effective diameter
c_r = 9.0/1000;            % (-) Wheel rolling friction coefficient
A_f = 2.3;                 % (m²) Vehicle frontal area
